...
def translate(tokenizer, texts) -> tuple:
    ...
    with torch.no_grad(): # отключаем подсчёт градиентов
        ...
        #---------------------------------------------------------------------------------------------------
    return tokens_count, latency


tokens_count, latency = translate(tokenizer, dataset["test"])

if not os.path.exists(RESULTS_DIR):
    os.makedirs(RESULTS_DIR)

with open(RESULTS_DIR + "tokens_count.json", mode='w', encoding='utf-8') as f: # открываем файл для записи (w — не побитовой)
    json.dump(tokens_count, f, ensure_ascii=False, indent=4) # сохраняем объект в файл f
with open(RESULTS_DIR + "tokenization_time.json", mode='w', encoding='utf-8') as f: # открываем файл для записи (w — не побитовой)
...
plt.savefig(f"{RESULTS_DIR}tokenization_time.png", dpi="figure", bbox_inches="tight", facecolor="white") # сохранение графика
